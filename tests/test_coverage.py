...
def test_coverage_calculation():
    ...
    assert report.coverage_details["REQ-100"].is_implemented == True
    assert report.coverage_details["REQ-100"].total_percentage == 100
    
    assert report.coverage_details["REQ-PARTIAL"].is_implemented == False
    assert report.coverage_details["REQ-PARTIAL"].total_percentage == 50
    assert len(report.coverage_details["REQ-PARTIAL"].matches) == 2
    
    assert report.coverage_details["REQ-MISSING"].is_implemented == False
    assert report.coverage_details["REQ-MISSING"].total_percentage == 0
    
    assert report.coverage_details["REQ-OVER"].is_implemented == True
